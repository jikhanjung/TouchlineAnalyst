...
## 후속 (같은 날): 분석 체크포인트에 원본 파일 키 추가

roll 보정으로 파노라마를 같은 경로에 재인코드하자, 기울어진 구 영상
기준의 .analysis.part.json 이 (키가 경로+파라미터뿐이라) 새 분석에
이어붙을 뻔했다 — ckpt_key 에 src_size/mtime 추가로 원본이 바뀌면
체크포인트 자동 무효.
